...
# JVM heap defaults.
# Override at startup when needed, for example:
#   YACY_JAVASTART_XMS=Xms16g YACY_JAVASTART_XMX=Xmx32g ./startYACY.sh
: "${YACY_JAVASTART_XMS:=Xms8g}"
: "${YACY_JAVASTART_XMX:=Xmx40g}"

#get javastart args
JAVA_ARGS="-$YACY_JAVASTART_XMS -server -Djava.awt.headless=true -Dfile.encoding=UTF-8";
...
if [ $PRINTONLY -eq 1 ];then
    ...
    echo "*******************************************************************************"
    echo "JVM heap: initial $YACY_JAVASTART_XMS, maximum $YACY_JAVASTART_XMX"
    if [ $DEBUG -eq 1 ] #debug
    then
